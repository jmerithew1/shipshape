...
import { queryResult, asQueryMock } from '../test/mock-query-result.js';
...
const queryMock = asQueryMock(pool.query);
...
describe('Accountability Service', () => {
  ...
  describe('date calculations', () => {
    it('handles workspace start date as Date object', async () => {
      ...
      queryMock
        .mockResolvedValueOnce(queryResult( [{ sprint_start_date: startDate }]))
        .mockResolvedValueOnce(queryResult( [{ id: personId }]))
        .mockResolvedValueOnce(queryResult( []))
        .mockResolvedValueOnce(queryResult( []))
        .mockResolvedValueOnce(queryResult( []))
  ...
  describe('per-person weekly retro due window (Thursday through Friday EOD)', () => {
    ...
    it('shows retro as due on Thursday', async () => {
      ...
      queryMock
        .mockResolvedValueOnce(queryResult( [{ id: 'plan-1', content: { type: 'doc', content: [{ type: 'text', text: 'My plan' }] } }]))
  ...
  describe('next-sprint lookahead', () => {
    it('checks both current AND next sprint for accountability', async () => {
      ...
      mockSetupQueries()
        ...
        .mockResolvedValueOnce(queryResult( [{ id: 'plan-1', content: { type: 'doc', content: [{ type: 'text', text: 'done' }] } }]))
        // Week 1 retro - exists (done) (today Jan 7 >= retroDueStr Jan 4)
        .mockResolvedValueOnce(queryResult( [{ id: 'retro-1', content: { type: 'doc', content: [{ type: 'text', text: 'done' }] } }]))
