#!/usr/bin/env bash
# Drift-detection demo (hashicorp/local provider only — touches NOTHING in the
# ship repo outside terraform/local/).
#
...
#
# Captures the raw output of every step into ./out/ for the audit record.
#
# Usage:  ./run-drift-demo.sh        (terraform must be on PATH)
...
# Apply is allowed HERE only: local files under ./managed/, no cloud resources.
run 06-apply        terraform apply -auto-approve -no-color

echo "########## BEFORE: on-disk content of the managed file ##########"
tee out/07-content-before.txt < managed/app-config.json
echo; echo
...
tee out/09-content-after-tamper.txt < managed/app-config.json
echo; echo

run 10-plan-drift   terraform plan -no-color   # expect: drift detected, 1 to add
...
# Reconcile: put the managed file back to the declared state.
run 12-apply-reconcile terraform apply -auto-approve -no-color

echo "########## AFTER RECONCILE: on-disk content ##########"
tee out/13-content-after-reconcile.txt < managed/app-config.json
echo; echo

run 14-plan-final   terraform plan -no-color   # expect: No changes.
