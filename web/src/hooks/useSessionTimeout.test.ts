...
// Mock fetch globally. Mocked responses must be Response-shaped (status +
// headers.get) because resetTimer's extend-session call goes through
// fetchWithCsrf, which reads res.status and res.headers.get('content-type');
// a bare { ok, json } mock throws there and masquerades as a network error,
// which forces logout via onTimeout.
...
describe('useSessionTimeout', () => {
  ...
  describe('Inactivity Timer', () => {
    ...
    it('does NOT call onTimeout if dismissed before 0', async () => {
      ...
      mockFetch.mockResolvedValue({ ok: true, status: 200, headers: { get: () => 'application/json' }, json: async () => ({ success: true }) });
...
describe('useSessionTimeout - Edge Cases', () => {
  ...
  it('handles resetTimer called when not showing warning', async () => {
    const onTimeout = vi.fn();
    (global.fetch as Mock).mockResolvedValue({ ok: true, status: 200, headers: { get: () => 'application/json' }, json: async () => ({ success: true }) });
