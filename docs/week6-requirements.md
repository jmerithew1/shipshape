...
| C3 | Drift: manually change a resource, terraform plan shows diff; destroy → apply from scratch; screenshots/log proof service came back identically | **PARTIAL (runbook ready; live run owner-gated)** | Credential-free half done (init/validate/fmt clean). Destroy→apply-from-scratch proven Week 5 (`terraform/render/out/13-14`). The live drift check + reconcile is scripted step-by-step in `docs/week6-terraform-drift-runbook.md` — the owner runs it with `RENDER_API_KEY` (a live secret) and captures the three plan receipts; the destroy half briefly takes prod down, so it's a maintenance-window action. |
