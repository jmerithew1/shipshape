...
| A9 | Existing regression suite passes; P95 latency, bundle size, per-route query counts within +10% of Part 1 baseline | **PARTIAL** | **Performance clause: MET.** `docs/week6-perf-regression.md` — bundle +1.21% raw / +2.43% gzip; per-route query counts within budget. P95 (citing the perf doc's own conservative headline pass D / `1fef201`, not the fastest run): `/api/issues` 181.7→106.8 ms (−41%), `/api/projects` 120.7→25.4 ms (−79%), `/api/auth/me` 83.3→19.8 ms (−76%); main-page cold queries 57→32. Absolute P95 varied up to ~4× run-to-run on this shared machine, so the **verdict** (well within the +10% budget on every route) is the load-bearing claim, not the exact millisecond figure. Vitest: api 872, web 165, sdk 82, cli 76, all green. **Regression-suite clause: MEASURED, not fully green (6 pre-existing frontend failures).** Full Playwright suite run **single-worker 2026-08-14**: **842 passed · 6 failed · 7 flaky (pass on retry) · 36 skipped**. The 5 FIXME specs ARE properly quarantined as `test.describe.fixme()` (the earlier "run unskipped" note was stale). The 7 flaky pass on retry — i.e. worker-contention/timing, not product bugs. The **6 hard failures are all pre-existing editor/interaction tests, unrelated to the Week-6 platform**: `backlinks` (remove-mention), `edge-cases` (simultaneous formatting), `inline-code` (Cmd/Ctrl+E toggle), `inline-comments` (Cmd+Shift+M), `session-timeout` (extend endpoint), `tables` (delete table). This branch touches no `web/` code except the one-line FileAttachment XSS sanitizer — none of the 6 involve attachments, so they are inherited, not introduced. The platform's OWN e2e is green (`oauth-pkce.spec.ts` 8/8; `prove-live.spec.ts` in CI). Fully green needs those 6 inherited editor tests fixed — a frontend effort outside the Week-6 platform scope, tracked but not done. |
