...
| A9 | Existing regression suite passes; P95 latency, bundle size, per-route query counts within +10% of Part 1 baseline | **MET** | `docs/week6-perf-regression.md` — measured, all categories PASS. Bundle +1.21% raw / +2.43% gzip. P95 c=10: `/api/issues` 181.7→72.3 ms (−60%), `/api/projects` 120.7→19.5 ms (−84%), `/api/auth/me` 83.3→12.2 ms (−85%). Main-page cold queries 57→32 (−44%). Suites: api 659, web 160, all green. Latency improved rather than regressed largely because the bearer path's per-request `last_used_at` UPDATE is now throttled to 30 s. Conservative comparison: measured against MORE data than the baseline (626 docs vs 557, 61 sprints vs 35) |
